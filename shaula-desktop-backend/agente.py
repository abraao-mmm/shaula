...
class AgenteReflexivo:
    ...
    def _fluxo_estabilizacao(self, plano: dict) -> Tuple[str, str]:
        ...
        prompt = (
            ...
            f"O utilizador falou com você. Responda de forma extremamente breve e gentil, adotando o tom: {plano['tom']}. "
            "Não faça perguntas complexas. Sugira descanso mental em breve."
        )
    ...
    def _fluxo_reflexao_profunda(self, entrada: str, contexto: str, plano: dict) -> Tuple[str, str]:
        """Mergulho profundo em tópicos complexos ou existenciais."""
        prompt = (
            f"### MODO DE REFLEXÃO PROFUNDA ###\n"
            f"Contexto de Identidade: {contexto}\n"
            f"Tom de voz a adotar: {plano['tom']}\n\n"
            f"Faça uma análise profunda, crítica e filosófica da seguinte entrada do Abraão: '{entrada}'"
        )
        resposta = self.obter_resposta_llm(prompt, max_tokens=plano["max_tokens"])
        # Aqui, no futuro, você pode acionar módulos paralelos de ruminação
    ...
    def _fluxo_tatico(self, entrada: str, contexto: str, plano: dict) -> Tuple[str, str]:
        ...
        prompt = (
            f"### MODO TÁTICO/FOCO ###\n"
            f"Seja direta, técnica e evite floreios. Tom: {plano['tom']}\n\n"
            f"Entrada: '{entrada}'"
        )
        resposta = self.obter_resposta_llm(prompt, max_tokens=plano["max_tokens"])
