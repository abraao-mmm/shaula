# backend/agente.py (VERSÃO FINAL 100% COMPLETA)
...
class AgenteReflexivo:
    ...
    def __init__(self, usuario_atual: Usuario, gerenciador: GerenciadorDeUsuarios, console_log: Console):
        ...
        self.aguardando_senha_texto: bool = False # NOVO: Controla o modo de digitação
...
""" }
    ...
    def processar_entrada_do_utilizador(self, entrada_usuario: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        # --- Lógica de Inventário (Fuzzy Matching) ---
            ...
            # Se a ação foi um sucesso (não precisou de confirmação e não deu erro)
            if sucesso_geral: 
                self.totem.sucesso()
                # Em vez de só formatar, vamos pedir à LLM uma resposta criativa
                prompt_criativo = (
                    f"Você é a Shaula. Diga ao Abraão que a ação de inventário dele foi um sucesso. "
                    f"O resultado foi: '{' '.join(respostas_feedback)}'. "
                ...
                self.totem.erro()
                texto_final = " ".join(respostas_feedback)
            ...
            texto_final_consulta = self.inventario_manager.consultar_estoque(item_nome)
            
            if "não encontrei" in texto_final_consulta: 
                self.totem.erro()
                return self._formata_resposta_direta(texto_final_consulta), "Consulta de inventário concluída."
            else: 
                self.totem.sucesso()
                # --- MUDANÇA AQUI ---
                # Pedir à LLM uma resposta criativa
                prompt_criativo = (
                    f"Você é a Shaula. Diga ao Abraão o resultado da consulta de inventário dele: '{texto_final_consulta}'. "
    ...
    def _processar_conversa_normal(self, entrada_usuario: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        # 2. Gera o prompt de persona
        prompt_persona = self.personalidade.gerar_descricao_persona_dinamica(self.usuario_atual)
        
        # 3. Cria o prompt final com memória e instruções anti-genéricas
        prompt_final = (
            f"{prompt_persona}\n\n"
            f"### CONTEXTO DA MEMÓRIA RECENTE (Nossas últimas trocas):\n{contexto_memoria}\n\n"
            f"### TAREFA IMEDIATA\n"
            f"- {self.usuario_atual.nome} acabou de dizer: '{escape(entrada_usuario)}'\n"
            f"- Responda a ele. **NÃO SEJA GENÉRICA.** Use seu tom curioso e carismático.\n"
            f"- **NUNCA** diga 'não tenho acesso a conversas passadas'. Você TEM acesso ao contexto acima. Use-o para dar uma resposta profunda (ou não) e relevante.\n"
            f"- **NÃO** termine sua resposta com uma pergunta genérica (como 'o que mais?', 'em que posso ajudar?'). Em vez disso, adicione um insight ou uma continuação natural da conversa."
        )
        
        resposta_dict = self.obter_resposta_llm(prompt_final)
        texto_final = resposta_dict.get("conteudo", "Não consegui responder.")
        
        # O _formata_resposta_direta é muito simples. Vamos pular ele e formatar aqui.
        return json.dumps({"ferramenta": "resposta_direta_streaming", "parametros": {"texto_final": texto_final}}), "Fluxo de conversa normal concluído."
    ...
    # Adicione este método inteiro em qualquer lugar dentro da classe AgenteReflexivo
